    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            ...
            if to_state == "open":
                ...
                recovery_times.append((ts - last_open_ts) * 1000.0)
                last_open_ts = None
    if recovery_times:
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        if scenario.name == "primary_timeout_100":
            passed = result.circuit_open_count > 0 and result.fallback_success_rate >= 0.85
        elif scenario.name == "all_healthy":
            passed = result.availability == 1.0 and result.circuit_open_count == 0
        elif scenario.name == "primary_flaky_50":
            passed = result.availability >= 0.85 and result.circuit_open_count > 0
        elif scenario.name == "both_failing_static":
            passed = result.static_fallbacks > 0
        else:
